...
m = 0
Axialq = 1.0
computationID =  @sprintf("3DSingleVBSmooth_N%d_m%d_q%.2f_qA%.2f_Urdis%.2f_VOFdis%.2f",N,m,q,Axialq,disturb,VOFdisturb)
...
function puresim!(sim,duration,recostep)
    ...
    iTime = 0
    jldsave("JLD2/"*computationID*"VelVOF_"*string(iTime)*".jld2"; u=Array(sim.flow.u))
    ii = 0
...
function multiphaseQVortex(NN; Re=4000, T=Float32, mem=Array)
    ...
    return WaterLily.TwoPhaseSimulation(
        ...
        U=U, Δt=0.01, ν=ν, T=T,perdir=(1,2,3),uλ=uλ,mem=mem
    )
end
